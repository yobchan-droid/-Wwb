...
export default function App() {
  ...
  const handleAudioImport = async (file: File) => {
    ...
    // Client space restriction (~40MB to keep processing prompt within reasonable request scale)
    if (file.size > 40 * 1024 * 1024) {
      setApiError("檔案容量過大！建議上傳 40MB 以下的音訊檔，以確保 Gemini AI 能夠順利進行轉錄。");
      return;
    }
    ...
    <div id="app-root" className="flex h-screen w-full bg-[#F8FAFC] text-slate-900 font-sans overflow-hidden select-none">
      ...
      <main className="flex-1 flex flex-col h-full min-w-0">
        ...
        <div className="flex-1 p-8 grid grid-cols-2 gap-8 overflow-hidden min-h-0">
              ...
              {isTranscribing ? (
                ...
                <div className="flex-1 flex flex-col justify-center items-center text-center p-6 text-slate-400 h-full">
                  ...
                  >
                    ...
                    <span className="text-[10px] text-slate-400">支援 MP3, WAV, M4A, AAC 等</span>
                  </div>
          ...
          {/* RIGHT SUB-GRID: AI Summary Markdown View */}
          <section className="flex flex-col min-h-0">
            <div className="flex items-center justify-between mb-3 shrink-0">
              <h2 className="text-xs font-bold text-slate-400 uppercase tracking-widest">
                AI 會議摘要
            ...
            {isGeneratingSummary ? (
              ...
              <div className="flex-1 bg-white border border-slate-200 rounded-2xl p-6 overflow-y-auto shadow-sm">
                <div className="markdown-body prose prose-slate max-w-none text-slate-700 text-sm leading-relaxed text-justify selection:bg-indigo-100">
                  <Markdown>{summary}</Markdown>
                </div>
                <div className="mt-6 pt-4 border-t border-slate-100 flex items-center justify-between text-[10px] text-slate-400 font-mono uppercase tracking-wider shrink-0">
