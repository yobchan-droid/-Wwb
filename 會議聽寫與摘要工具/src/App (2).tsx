...
export default function App() {
  ...
  // Text-to-Speech (TTS) and Iframe detection states
  const [isPlayingTTS, setIsPlayingTTS] = useState(false);
  const [ttsTarget, setTtsTarget] = useState<"transcript" | "summary" | null>(null);
  ...
  // Detect if running inside an iframe and manage TTS states
  useEffect(() => {
  ...
  // When active meeting changes, stop any speech playback
  useEffect(() => {
    if (window.speechSynthesis) {
      window.speechSynthesis.cancel();
    }
  ...
  // Browser Speech Synthesis: Text-to-Speech (TTS)
  const handleToggleTTS = (text: string, target: "transcript" | "summary") => {
    if (!window.speechSynthesis) {
      alert("此瀏覽器不支援語音合成服務（Text-to-Speech）。");
      return;
    }

    if (isPlayingTTS && ttsTarget === target) {
      window.speechSynthesis.cancel();
      setIsPlayingTTS(false);
      setTtsTarget(null);
      return;
    ...
    // Clean text: strip markdown patterns for clean speaking output
    ...
    // Voice mapping logic matching selected target language
    const langVoiceMap: { [key: string]: string } = {
      "zh-TW": "zh-TW",
      "en-US": "en-US",
      "ja-JP": "ja-JP",
      "zh-CN": "zh-CN"
    };
    utterance.lang = langVoiceMap[selectedLanguage] || "zh-TW";
  ...
  return (
    <div id="app-root" className="flex h-screen w-full bg-[#F8FAFC] text-slate-900 font-sans overflow-hidden select-none">
      ...
      <main className="flex-1 flex flex-col h-full min-w-0">
        ...
        {/* Browser Iframe Security Warning Notice Bar */}
        {isInIframe && (
          <div className="mx-8 mt-6 p-4 bg-amber-50/60 border border-amber-100 rounded-xl flex items-center justify-between shadow-sm shrink-0 gap-4">
            <div className="flex items-start gap-2.5">
              <Info className="h-4 w-4 text-amber-600 shrink-0 mt-0.5 animate-pulse" />
              <div className="text-xs text-amber-800 leading-normal">
                <span className="font-bold">瀏覽器安全提醒：</span>
                即時語音辨識功能需要取得麥克風授權。由於瀏覽器安全限制，在開發者內嵌框架 (iframe) 中通常會被阻擋。
                強烈建議您點擊右側按鈕<b>在新分頁中開啟</b>，即可完美暢用即時轉語音功能！
              </div>
            </div>
            <button
              onClick={() => {
              ...
              className="text-[11px] font-bold text-amber-700 hover:text-amber-800 underline bg-white border border-amber-250 hover:bg-amber-50 px-3 py-1.5 rounded-lg transition-all shrink-0 cursor-pointer shadow-sm active:scale-98"
            >
              在新分頁打開 ↗
            </button>
          </div>
              ...
              <div className="flex items-center gap-3">
                ...
                {transcript && (
                  <div className="flex items-center gap-2.5">
                    <button
                      onClick={() => handleToggleTTS(transcript, "transcript")}
                      className={`text-[11px] font-bold flex items-center gap-1 transition-all cursor-pointer ${
                        isPlayingTTS && ttsTarget === "transcript"
                          ? "text-red-500 hover:text-red-750 animate-pulse"
                          : "text-slate-500 hover:text-indigo-650"
                      }`}
                      title={isPlayingTTS && ttsTarget === "transcript" ? "停止朗讀聽寫記錄" : "將逐字稿進行語音朗讀"}
                    >
                      {isPlayingTTS && ttsTarget === "transcript" ? (
                        <>
            ...
            <div className="flex items-center justify-between mb-3 shrink-0 border-b border-slate-200 pb-1.5">
              ...
              {activeTab === "summary" ? (
                summary && !isGeneratingSummary && (
                  <div className="flex items-center gap-2.5">
                    <button
                      onClick={() => handleToggleTTS(summary, "summary")}
                      className={`text-[11px] font-bold flex items-center gap-1 transition-all cursor-pointer ${
                        isPlayingTTS && ttsTarget === "summary"
                          ? "text-red-500 hover:text-red-750 animate-pulse"
                          : "text-slate-500 hover:text-indigo-650"
                      }`}
                      title={isPlayingTTS && ttsTarget === "summary" ? "停止朗讀會議摘要" : "將 AI 摘要報告進行語音朗讀"}
                    >
                      {isPlayingTTS && ttsTarget === "summary" ? (
                        <>
                          <VolumeX className="h-3.5 w-3.5 text-red-500" />
